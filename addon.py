...
addon = xbmcaddon.Addon(id='plugin.video.ukrainskietv')
...
chCodes=['ukrainskietv-cvk']


channels={
    'televsesvit':['Телевсесвіт','',img_path+'https://www.televsesvit.ua/site/img/logo_white.png','',0],
    'prtu_lviv':['ПравдаТУТ Львів','',img_path+'https://lowa8026-cmyk.github.io/prtu_lviv.png','',0],
    'prtu_kyiv':['ПравдаТУТ Київ','',img_path+'https://lowa8026-cmyk.github.io/prtu_kyiv.png','',0],
    'priamyi':['Прямий','',img_path+'https://lowa8026-cmyk.github.io/priamyi.png','',0],
    'ua_razom':['Єдині новини','617',img_path+'https://lowa8026-cmyk.github.io/ua_razom.png','',0],
    'd1_dn':['D1 Дніпро','',img_path+'https://lowa8026-cmyk.github.io/d1_dn.png','',0],
}
...
def ytWatch(v):
    ...
    if stream_url!='':
        ...
        xbmcgui.Dialog().notification('Ukraińskie TV. Bez opłat', 'Brak transmisji', xbmcgui.NOTIFICATION_INFO)
...
def ukrainskietv(c,s=None,e=None):
    stream_url=''
    protocol=''
    ...
    if stream_url !='' and protocol !='':
        ...
        xbmcgui.Dialog().notification('Ukraińskie TV. Bez opłat', info, xbmcgui.NOTIFICATION_INFO)
        xbmcplugin.setResolvedUrl(addon_handle, False, xbmcgui.ListItem())
...
def ewtn():
    ...
    try:
        ...
        xbmcgui.Dialog().notification('Ukraińskie TV. Bez opłat', 'Brak źródła', xbmcgui.NOTIFICATION_INFO)
        xbmcplugin.setResolvedUrl(addon_handle, False, xbmcgui.ListItem())
...
def enpl():
    ...
    if 'videoPrimaryUrl' in resp:
        ...
        if resp['player']=='pfp':
            ...
            xbmcgui.Dialog().notification('Ukraińskie TV. Bez opłat', 'Niezidentyfikowany player', xbmcgui.NOTIFICATION_INFO)
            xbmcplugin.setResolvedUrl(addon_handle, False, xbmcgui.ListItem())
...
def listM3U():
    ...
    if file_name == '' or path_m3u == '':
        xbmcgui.Dialog().notification('Ukraińskie TV. Bez opłat', 'Podaj nazwę pliku oraz katalog docelowy.', xbmcgui.NOTIFICATION_ERROR)
        return
    xbmcgui.Dialog().notification('Ukraińskie TV. Bez opłat', 'Generuję listę M3U.', xbmcgui.NOTIFICATION_INFO)
    data = '#EXTM3U\n'
    for c in list(channels.keys()):
        cData=channels[c]
        name=cData[0]
        ...
        cuTime=str(cData[4])
        if cu=='c':
            data += '#EXTINF:0 tvg-id="%s" tvg-logo="%s" group-title="Ukraińskie TV. Bez opłat" catchup="append" catchup-source="&s={utc:Y-m-dTH:M:S}&e={utcend:Y-m-dTH:M:S}" catchup-days="%s",%s\nplugin://plugin.video.ukrainskietv?mode=playLive&chan=%s\n' %(name,img,cuTime,name,c)
        else:
            data += '#EXTINF:0 tvg-id="%s" tvg-logo="%s" group-title="Ukraińskie TV. Bez opłat" ,%s\nplugin://plugin.video.ukrainskietv?mode=playLive&chan=%s\n' %(name,img,name,c)
        
        f = xbmcvfs.File(path_m3u + file_name, 'w')
        f.write(data)
        f.close()
        xbmcgui.Dialog().notification('Ukraińskie TV. Bez opłat', 'Wygenerowano listę M3U', xbmcgui.NOTIFICATION_INFO)
    else:
        xbmcgui.Dialog().notification('Ukraińskie TV. Bez opłat', 'Błąd przy generowaniu listy M3U', xbmcgui.NOTIFICATION_INFO)
